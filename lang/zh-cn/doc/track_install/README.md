...
* [2. 执行Install计测(可指定选项)](#track_install_optional)
...
此外在`SFSafariViewController`进行Install计测后为关闭`SFSafariViewController`，请在`-(BOOL)application:openURL:sourceApplication:annotation:`方法内执行`[CYZFox handleOpenURL:url]`。
...
> ※ 未指定引数呼出`trackInstall`方法时，将会优先F.O.X管理页面上的设置内容。需设置Cookie计测时自动跳转的URL时，请参考本章中以下说明。

> ※ APP启动时呼出`didFinishLaunchingWithOptions`方法中未执行代码的状态下实行安装成果型广告时，必须向广告代理店或媒体公司负责人进行说明。未正确进行计测就开始安装成果型广告，可能会产生超过计测安装数的广告费用。

<div id="track_install_optional"></div>
## 2. 执行Install计测(option选项)

想要用callback来获取Install计测完成信息、跳转到特定URL、用APP动态生成URL时，请使用以下[CYZFoxTrackOption](../sdk_api/README.md#CYZFoxoption)类。<br>
